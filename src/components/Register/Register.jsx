...
async function signUp(values) { // values are sent by formik we can recieve them here 
  setisLoading(true);
try{
   const {data} = await axios.post("https://localhost:7161/api/Account/Register",values); // send the values with the post method in the body which are the user object updated with new values 
...
const signupSchema = Yup.object().shape({
    ...
    confirmPassword: Yup.string().required("Re-Password is a required field").oneOf([Yup.ref("password")],"Re-password does not match the original one"),

    phoneNumber: Yup.string().required("Phone is a required field").matches(/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/,"Phone number is invalid")
